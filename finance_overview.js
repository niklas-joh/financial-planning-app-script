...
function createFinancialOverview() {
  ...
  let overviewSheet;
  try {
    overviewSheet = ss.getSheetByName("Overview");
    if (!overviewSheet) {
      overviewSheet = ss.insertSheet("Overview");
    } else {
      // Clear existing content but preserve formatting
      overviewSheet.getRange(1, 1, overviewSheet.getMaxRows(), overviewSheet.getMaxColumns()).clearContent();
    }
  } catch (e) {
    overviewSheet = ss.insertSheet("Overview");
  }
...
function formatOverviewSheet(sheet) {
  ...
  currencyColumns.forEach(col => {
    sheet.getRange(2, col, lastRow - 1, 1).setNumberFormat('€#,##0.00;[Red]-€#,##0.00');
  });
  
  // Format percentage columns
  const percentColumns = [12, 13, 14]; // Rate columns
  percentColumns.forEach(col => {
    sheet.getRange(2, col, lastRow - 1, 1).setNumberFormat('0.0%');
  });
  ...
  for (let i = 2; i <= lastRow; i += 2) {
    sheet.getRange(i, 1, 1, sheet.getLastColumn()).setBackground('#f9f9f9');
  }
}
...
function onOpen() {
  ...
  ui.createMenu('Financial Overview')
