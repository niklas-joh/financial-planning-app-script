...
function generateMonthlySpendingReport() {
  ...
  // Check if report sheet exists, if not create it
  let reportSheet;
  try {
    reportSheet = ss.getSheetByName("Monthly Report");
    if (!reportSheet) {
      reportSheet = ss.insertSheet("Monthly Report");
    } else {
      // Clear existing content but preserve formatting
      reportSheet.getRange(1, 1, reportSheet.getMaxRows(), reportSheet.getMaxColumns()).clearContent();
    }
  } catch (e) {
    reportSheet = ss.insertSheet("Monthly Report");
  }
